...
"ME_required_MJ_AM",
  ...
  for (i in 1:nruns) {
    ...
    for (month in 1:Num_timesteps) {
      ...
      ## UPDATE: Gamma offtake is now linear rather than exponential, as exp does not work well with high rates
      ...
      ## use Num_timesteps
      if (species == "cattle" ) {
        res_vec$Offtake_JF[month] <- sample(Gamma_JF, 1)*(1/Num_timesteps) * JF
        res_vec$Offtake_JM[month] <- sample(Gamma_JM, 1)*(1/Num_timesteps) * JM
        res_vec$Offtake_SubAF[month] <- sample(Gamma_SubAF, 1)*(1/Num_timesteps) * SubAF ## assuming subadults for >= 1 year
        res_vec$Offtake_SubAM[month] <- sample(Gamma_SubAM, 1)*(1/Num_timesteps) * SubAM ## assuming subadults for >= 1 year
        res_vec$Offtake_AF[month] <- sample(Gamma_AF, 1)*(1/Num_timesteps) * AF
        res_vec$Offtake_AM[month] <- sample(Gamma_AM, 1)*(1/Num_timesteps) * AM
        res_vec$Offtake_Ox[month] <- sample(Gamma_Ox, 1)*(1/Num_timesteps) * Ox
      } 
      ## separated for small ruminants and poultry ## Different to cattle because they are in the second age sex group < 1 year
      if (species == "poultry" || species == "smallruminants" || species == "swine") {
        res_vec$Offtake_JF[month] <- sample(Gamma_JF, 1)*(1/Beta_JF) * JF ## assuming <12 months
        res_vec$Offtake_JM[month] <- sample(Gamma_JM, 1)*(1/Beta_JM) * JM ## assuming <12 months
        res_vec$Offtake_SubAF[month] <- sample(Gamma_SubAF, 1)*(1/Beta_SubAF) * SubAF ## offtake for subadults assuming <12 months
        res_vec$Offtake_SubAM[month] <- sample(Gamma_SubAM, 1)*(1/Beta_SubAM) * SubAM ## offtake for subadults assuming <12 months
        res_vec$Offtake_AF[month] <- sample(Gamma_AF, 1)*(1/Num_timesteps) * AF
        res_vec$Offtake_AM[month] <- sample(Gamma_AM, 1)*(1/Num_timesteps) * AM
      ...
      if (species == "swine"){
        res_vec$Offtake_GF[month] <- sample(Gamma_GF, 1)*(1/Beta_GF) * GF ## offtake for growers assuming <12 months
        res_vec$Offtake_GM[month] <- sample(Gamma_GM, 1)*(1/Beta_GM) * GM ## offtake for growers assuming <12 months
        res_vec$Offtake_GltF[month] <- sample(Gamma_GltF, 1)*(1/Beta_GltF) * GltF ## offtake for growers assuming <12 months
        res_vec$Offtake_GltM[month] <- sample(Gamma_GltM, 1)*(1/Beta_GltM) * GltM ## offtake for growers assuming <12 months
        
      }
      
      if (species == "equids") { 
        res_vec$Offtake_JF[month] <- sample(Gamma_JF, 1)*(1/Beta_JF) * JF
        res_vec$Offtake_JM[month] <- sample(Gamma_JM, 1)*(1/Beta_JM) * JM
        res_vec$Offtake_SubAF[month] <- sample(Gamma_SubAF, 1)*(1/Num_timesteps) * SubAF ## offtake for subadults assuming <12 months
        res_vec$Offtake_SubAM[month] <- sample(Gamma_SubAM, 1)*(1/Num_timesteps) * SubAM ## offtake for subadults assuming <12 months
        res_vec$Offtake_AF[month] <- sample(Gamma_AF, 1)*(1/Num_timesteps) * AF
        res_vec$Offtake_AM[month] <- sample(Gamma_AM, 1)*(1/Num_timesteps) * AM
    ...
    ##  Now remove culls
      ...
      if (species == "equids") {
        ## commercial
        res_vec$working_hours_commercial_AF[month] <- commercial_hours_AF + (res_vec$Num_AF[month] * sample(prop_commercial_AF, 1)) * days_worked_month_commercial_AF * (sample(WH_commercial_AF_ideal,1) / 60)
        res_vec$working_hours_commercial_AM[month] <- commercial_hours_AM + (res_vec$Num_AM[month] * sample(prop_commercial_AM, 1)) * days_worked_month_commercial_AM * (sample(WH_commercial_AM_ideal,1) / 60)
        ...
        ## unpaid
        res_vec$working_hours_unpaid_SubAF[month] <- unpaid_hours_SubAF + (res_vec$Num_SubAF[month] * sample(prop_unpaid_SubAF,1)) * days_worked_month_unpaid_SubAF * (sample(WH_unpaid_SubAF_ideal,1) / 60)
        res_vec$working_hours_unpaid_SubAM[month] <- unpaid_hours_SubAM + (res_vec$Num_SubAM[month] * sample(prop_unpaid_SubAM,1)) * days_worked_month_unpaid_SubAM * (sample(WH_unpaid_SubAM_ideal,1) / 60)
        res_vec$working_hours_unpaid_AF[month] <- unpaid_hours_AF + (res_vec$Num_AF[month] * sample(prop_unpaid_AF,1)) * days_worked_month_unpaid_AF * (sample(WH_unpaid_AF_ideal,1) / 60)
        res_vec$working_hours_unpaid_AM[month] <- unpaid_hours_AM + (res_vec$Num_AM[month] * sample(prop_unpaid_AM,1)) * days_worked_month_unpaid_AM * (sample(WH_unpaid_AM_ideal,1) / 60)
        ...
        ## combined
        ## combined
        res_vec$working_hours_combined_SubAF[month] <- combined_hours_SubAF + (res_vec$Num_SubAF[month] * sample(prop_combined_hhcom_SubAF,1)) * days_worked_month_combined_SubAF * (sample(WH_combined_hhcom_SubAF_ideal,1) / 60)
        res_vec$working_hours_combined_SubAM[month] <- combined_hours_SubAM + (res_vec$Num_SubAM[month] * sample(prop_combined_hhcom_SubAM,1)) * days_worked_month_combined_SubAM * (sample(WH_combined_hhcom_SubAM_ideal,1) / 60)
        res_vec$working_hours_combined_AF[month] <- combined_hours_AF + (res_vec$Num_AF[month] * sample(prop_combined_hhcom_AF,1)) * days_worked_month_combined_AF * (sample(WH_combined_hhcom_AF_ideal,1) / 60)
        res_vec$working_hours_combined_AM[month] <- combined_hours_AM + (res_vec$Num_AM[month] * sample(prop_combined_hhcom_AM,1)) * days_worked_month_combined_AM * (sample(WH_combined_hhcom_AM_ideal,1) / 60)
    ...
    if (species == "cattle") {
      ...
      if (species == "cattle" || species == "smallruminants" ||species == "swine") {
        ...
        res_vec$Purchased_Feed_KG_JF[month] <- res_vec$Feed_required_KG_JF[month] * sample(prpn_lskeepers_purch_feed,1) * sample(prpn_feed_paid_for,1)
        res_vec$Purchased_Feed_KG_JM[month] <- res_vec$Feed_required_KG_JM[month] * sample(prpn_lskeepers_purch_feed,1) * sample(prpn_feed_paid_for,1)
        res_vec$Purchased_Feed_KG_SubAF[month] <- res_vec$Feed_required_KG_SubAF[month] * sample(prpn_lskeepers_purch_feed,1) * sample(prpn_feed_paid_for,1)
        res_vec$Purchased_Feed_KG_SubAM[month] <- res_vec$Feed_required_KG_SubAM[month] * sample(prpn_lskeepers_purch_feed,1) * sample(prpn_feed_paid_for,1)
        res_vec$Purchased_Feed_KG_AF[month] <- res_vec$Feed_required_KG_AF[month] * sample(prpn_lskeepers_purch_feed,1) * sample(prpn_feed_paid_for,1)
        res_vec$Purchased_Feed_KG_AM[month] <- res_vec$Feed_required_KG_AM[month] * sample(prpn_lskeepers_purch_feed,1) * sample(prpn_feed_paid_for,1)
      ...
      if (species == "equids"){  
        ...
        res_vec$Purchased_Feedpartial_KG_JF[month] <- res_vec$Feed_required_KG_JF[month] * sample(prpn_lskeepers_partial_feed, 1) * sample(lskeepers_partial_feed_prpn_fd_purch, 1)
        res_vec$Purchased_Feedpartial_KG_JM[month] <- res_vec$Feed_required_KG_JM[month] * sample(prpn_lskeepers_partial_feed, 1) * sample(lskeepers_partial_feed_prpn_fd_purch, 1)
        res_vec$Purchased_Feedpartial_KG_SubAF[month] <- res_vec$Feed_required_KG_SubAF[month] * sample(prpn_lskeepers_partial_feed, 1) * sample(lskeepers_partial_feed_prpn_fd_purch, 1)
        res_vec$Purchased_Feedpartial_KG_SubAM[month] <- res_vec$Feed_required_KG_SubAM[month] * sample(prpn_lskeepers_partial_feed, 1) * sample(lskeepers_partial_feed_prpn_fd_purch, 1)
        res_vec$Purchased_Feedpartial_KG_AF[month] <- res_vec$Feed_required_KG_AF[month] * sample(prpn_lskeepers_partial_feed, 1) * sample(lskeepers_partial_feed_prpn_fd_purch, 1)
        res_vec$Purchased_Feedpartial_KG_AM[month] <- res_vec$Feed_required_KG_AM[month] * sample(prpn_lskeepers_partial_feed, 1) * sample(lskeepers_partial_feed_prpn_fd_purch, 1)
        ...
        res_vec$Purchased_Feed_KG_JF[month] <- res_vec$Feed_required_KG_JF[month] * sample(prpn_lskeepers_all_handfeed, 1) * sample(lskeepers_all_handfeed_prpn_fd_purch, 1)
        res_vec$Purchased_Feed_KG_JM[month] <- res_vec$Feed_required_KG_JM[month] * sample(prpn_lskeepers_all_handfeed, 1) * sample(lskeepers_all_handfeed_prpn_fd_purch, 1)
        res_vec$Purchased_Feed_KG_SubAF[month] <- res_vec$Feed_required_KG_SubAF[month] * sample(prpn_lskeepers_all_handfeed, 1) * sample(lskeepers_all_handfeed_prpn_fd_purch, 1)
        res_vec$Purchased_Feed_KG_SubAM[month] <- res_vec$Feed_required_KG_SubAM[month] * sample(prpn_lskeepers_all_handfeed, 1) * sample(lskeepers_all_handfeed_prpn_fd_purch, 1)
        res_vec$Purchased_Feed_KG_AF[month] <- res_vec$Feed_required_KG_AF[month] * sample(prpn_lskeepers_all_handfeed, 1) * sample(lskeepers_all_handfeed_prpn_fd_purch, 1)
        res_vec$Purchased_Feed_KG_AM[month] <- res_vec$Feed_required_KG_AM[month] * sample(prpn_lskeepers_all_handfeed, 1) * sample(lskeepers_all_handfeed_prpn_fd_purch, 1)
        ...
        if (species == "swine"){
          ...
          res_vec$Purchased_Feed_KG_GF[month] <- res_vec$Feed_required_KG_GF[month] * sample(prpn_lskeepers_purch_feed,1) * sample(prpn_feed_paid_for,1)
          res_vec$Purchased_Feed_KG_GM[month] <- res_vec$Feed_required_KG_GM[month] * sample(prpn_lskeepers_purch_feed,1) * sample(prpn_feed_paid_for,1)
          res_vec$Purchased_Feed_KG_GltF[month] <- res_vec$Feed_required_KG_GltF[month] * sample(prpn_lskeepers_purch_feed,1) * sample(prpn_feed_paid_for,1)
          res_vec$Purchased_Feed_KG_GltM[month] <- res_vec$Feed_required_KG_GltM[month] * sample(prpn_lskeepers_purch_feed,1) * sample(prpn_feed_paid_for,1)
      ...
      if (species == "cattle"){
        ...
        res_vec$Purchased_Feed_KG_Ox[month] <- res_vec$Feed_required_KG_Ox[month] * sample(prpn_lskeepers_purch_feed,1) * sample(prpn_feed_paid_for,1)
