  ...
  for (i in 1:nruns) {
    ...
    for (month in 1:Num_timesteps) {
      ...
      if (species == "poultry" || species == "smallruminants") {
        res_vec$Offtake_JF[month] <- (1-(1-sample(Gamma_JF, 1))^(1/Beta_JF)) * JF
        res_vec$Offtake_JM[month] <- (1-(1-sample(Gamma_JM, 1))^(1/Beta_JM)) * JM
        res_vec$Offtake_SubAF[month] <- (1-(1-sample(Gamma_SubAF, 1))^(1/Beta_SubAF)) * SubAF ## offtake for subadults assuming <12 months
        res_vec$Offtake_SubAM[month] <- (1-(1-sample(Gamma_SubAM, 1))^(1/Beta_SubAM)) * SubAM ## offtake for subadults assuming <12 months
        res_vec$Offtake_AF[month] <- (1-(1-sample(Gamma_AF, 1))^(1/Num_timesteps)) * AF
        res_vec$Offtake_AM[month] <- (1-(1-sample(Gamma_AM, 1))^(1/Num_timesteps)) * AM
